...
return (
  <>
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col lg:flex-row mx-4 md:mx-8 lg:mx-20 gap-8">
        <div className="flex flex-col lg:flex-row w-full lg:w-1/2 gap-8">
          <div className="flex flex-col">
            {product.photos.map((photo, index) => (
              <img
                ...
                className="hidden md:block w-full max-w-lg m-2"
              />
            ))}
          </div>
        ...
        <div className="flex flex-col w-full lg:w-1/2 gap-4">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="inline-block h-2 min-h-[1.7em] w-[1.5px] self-stretch bg-[#9F9F9F]"></div>
            <div className="text-[#9F9F9F]">{product.totalreviews} Customer Reviews</div>
          </div>
          <div className="line-clamp-4 w-full lg:w-3/4">{product.description}</div>
          <div className="flex gap-4 items-center">
            ...
            <button
              onClick={addToCart}
              className="border border-[#000000] flex py-2 px-8 font-medium justify-center gap-4 items-center rounded-md"
            >
              Add to Cart
            </button>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex">
              <div className="w-28 text-[#9F9F9F]">Share</div>
      ...
      <div className="flex flex-col gap-4 justify-center">
        <div className="text-center text-2xl font-semibold">Description</div>
        <div className="mx-4 md:mx-8 lg:mx-20 text-[#9F9F9F]">{product.description}</div>
      </div>
          ...
          {relatedProducts.map((relatedProduct) => (
            <div key={relatedProduct._id} className="relative border flex flex-col group">
              ...
              <div className="flex flex-col gap-1 h-2/6 bg-[#F4F5F7] p-5">
                ...
                <div className="text-[#898989] font-medium text-sm">{relatedProduct.subheading}</div>
                <div className="flex justify-between">
                  <div className="text-[#3A3A3A] font-semibold text-lg">Rp {relatedProduct.price}.00</div>
                  {product.realprice && (
                    <div className="text-[#B0B0B0] line-through">Rp {relatedProduct.realprice}.00</div>
                  )}
                </div>
              </div>
              <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="flex flex-col gap-4">
                  <button
                    className="bg-white text-[#B88E2F] py-2 px-10 font-semibold rounded flex items-center"
                    onClick={addToCart}
                  >
                    Add to Cart
                  </button>
                  <div className="flex justify-between">
                    <button className="flex text-white items-center" onClick={() => handleShare(product)}>
